...
class ApiService {
  ...
  static Future<List<dynamic>> getSensorHistoryRange(String kotak, String dari, String sampai) async {
    try {
      final r = await http.get(
        Uri.parse('$baseUrl/api/sensor/history?kotak=$kotak&dari=$dari&sampai=$sampai'),
  ...
  // Export CSV — returns URL to download
  static String getExportCsvUrl(String kotak, String dari, String sampai) {
    return '$baseUrl/api/sensor/export?kotak=$kotak&dari=$dari&sampai=$sampai';
  }
}
      ...
      body: Center(
        child: SingleChildScrollView(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 12),
            Text('Tamu hanya dapat melihat data monitoring', style: TextStyle(fontSize: 10, color: AppColors.grey)),
          ]),
        ),
      ),
    );
...
// ══════════════════════════════════════
// HELPER: Filter data kotor
// pH valid: 0–14, TDS valid: > 0
// ══════════════════════════════════════
bool _isValidPh(double v)  => v >= 0 && v <= 14;
bool _isValidTds(double v) => v > 0;

// ══════════════════════════════════════
// TAB 2 - SENSOR (REVISED)
// ══════════════════════════════════════
...
class _SensorState extends State<SensorPage> with SingleTickerProviderStateMixin {
  ...
  // Filter tanggal per kotak
  final Map<String, DateTime> _dariDate  = {};
  final Map<String, DateTime> _sampaiDate = {};
  ...
  void initState() {
    ...
    for (final k in ['kotak_1', 'kotak_2', 'kotak_3', 'kotak_4']) {
      _dariDate[k]   = now.subtract(const Duration(days: 6));
      _sampaiDate[k] = now;
    }
    _fetch();
  ...
  @override
  void dispose() { _tabController.dispose(); _timer?.cancel(); super.dispose(); }

  Future<void> _fetch() async {
    final s       = await ApiService.getSensorLatest();
    final setting = await ApiService.getPengaturan();
    for (final k in _kotakList) {
      final dari   = _fmtDate(_dariDate[k]!);
      final sampai = _fmtDate(_sampaiDate[k]!);
      final h = await ApiService.getSensorHistoryRange(k, dari, sampai);
      if (mounted) setState(() {
        _loading    = false;
        _sensor     = s;
        _setting    = setting;
  ...
  Future<void> _pickDate(String kotak, bool isDari) async {
    final initial = isDari ? _dariDate[kotak]! : _sampaiDate[kotak]!;
    ...
    if (picked != null && mounted) {
      setState(() {
        if (isDari) {
          _dariDate[kotak] = picked;
          if (picked.isAfter(_sampaiDate[kotak]!)) _sampaiDate[kotak] = picked;
        } else {
          _sampaiDate[kotak] = picked;
          if (picked.isBefore(_dariDate[kotak]!)) _dariDate[kotak] = picked;
        }
      });
      _fetch();
    }
  ...
  Widget build(BuildContext context) => Scaffold(
    backgroundColor: AppColors.bg,
    body: Column(children: [
      buildTopBar('DETAIL SENSOR', 'pH & TDS per Kotak'),
      Container(
        color: AppColors.card,
        child: TabBar(
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          _buildDateFilterRow(kotak),
          const SizedBox(height: 12),

          // ── Line chart pH ──
          _buildLineChart('📈 GRAFIK pH', history, 'ph', AppColors.primary,
            phMin: (_setting['ph_min'] ?? 5.5).toDouble(),
            phMax: (_setting['ph_max'] ?? 6.5).toDouble(),
          ),
          const SizedBox(height: 12),

          // ── Line chart TDS ──
          _buildLineChart('📈 GRAFIK TDS (ppm)', history, 'tds', AppColors.blue,
            tdsMin: (_setting['tds_min'] ?? 800).toDouble(),
            tdsMax: (_setting['tds_max'] ?? 1200).toDouble(),
          ),
          const SizedBox(height: 12),

          // ── Ringkasan statistik ──
          _buildStatistikCard(history, kotak),
        ]),
      ),
    );
  }
  ...
  Widget _buildDateFilterRow(String kotak) => Container(
    padding: const EdgeInsets.all(12),
    decoration: BoxDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.border)),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('📅 FILTER TANGGAL', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.grey)),
      const SizedBox(height: 10),
      Row(children: [
        Expanded(child: GestureDetector(
          onTap: () => _pickDate(kotak, true),
          child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Dari', style: TextStyle(fontSize: 9, color: AppColors.grey)),
              Text(_displayDate(_dariDate[kotak]!),
        ...
        Expanded(child: GestureDetector(
          ...
          child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Sampai', style: TextStyle(fontSize: 9, color: AppColors.grey)),
              Text(_displayDate(_sampaiDate[kotak]!),
                style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.text)),
            ]),
          ),
        )),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(children: const [
              Icon(Icons.download, color: AppColors.blue, size: 16),
              SizedBox(width: 4),
              Text('CSV', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.blue)),
            ]),
          ),
        ),
      ]),
  ...
  void _exportCsv(String kotak) {
    final dari   = _fmtDate(_dariDate[kotak]!);
    final sampai = _fmtDate(_sampaiDate[kotak]!);
    final url    = ApiService.getExportCsvUrl(kotak, dari, sampai);
    // Buka URL di tab baru — browser akan download CSV
    // ignore: avoid_web_libraries_in_flutter
    import_web_open(url);
  }
  ...
  Widget _buildLineChart(
    ...
    final filtered = history.where((e) {
      final v = (e['${field}_avg'] as num?)?.toDouble() ?? -1;
      if (field == 'ph')  return _isValidPh(v);
      if (field == 'tds') return _isValidTds(v);
      return true;
    }).toList();
    ...
    if (filtered.isEmpty) return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 20),
        const Center(child: Text('Belum ada data valid', style: TextStyle(fontSize: 11, color: AppColors.grey))),
        const SizedBox(height: 20),
      ]),
    );

    final values = filtered.map((e) => (e['${field}_avg'] as num).toDouble()).toList();
    double maxVal = values.reduce((a, b) => a > b ? a : b);
    double minVal = values.reduce((a, b) => a < b ? a : b);

    // Extend range agar garis batas terlihat
    if (phMin != null) minVal = [minVal, phMin! - 0.5].reduce((a, b) => a < b ? a : b);
    if (phMax != null) maxVal = [maxVal, phMax! + 0.5].reduce((a, b) => a > b ? a : b);
    if (tdsMin != null) minVal = [minVal, tdsMin! - 50].reduce((a, b) => a < b ? a : b);
    if (tdsMax != null) maxVal = [maxVal, tdsMax! + 50].reduce((a, b) => a > b ? a : b);

    final range = maxVal - minVal == 0 ? 1.0 : maxVal - minVal;
    const chartH = 110.0;

    double _y(double v) => chartH - ((v - minVal) / range * chartH);
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 12),
        SizedBox(
          height: chartH + 24, // +24 untuk label tanggal
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            // Y-axis labels
            SizedBox(width: 36, child: Column(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(maxVal.toStringAsFixed(field == 'tds' ? 0 : 1),
                  style: const TextStyle(fontSize: 8, color: AppColors.grey)),
                Text(((maxVal + minVal) / 2).toStringAsFixed(field == 'tds' ? 0 : 1),
                  style: const TextStyle(fontSize: 8, color: AppColors.grey)),
                Text(minVal.toStringAsFixed(field == 'tds' ? 0 : 1),
                  style: const TextStyle(fontSize: 8, color: AppColors.grey)),
            ...
            Expanded(child: CustomPaint(
              size: Size.fromHeight(chartH + 24),
              painter: _LineChartPainter(
                values: values,
                labels: filtered.map((e) {
                  final t = e['tanggal'].toString();
                  return t.length >= 10 ? t.substring(5) : t; // MM-DD
                }).toList(),
                color: color,
                minVal: minVal,
                maxVal: maxVal,
        ...
        // Legend batas normal
        if (phMin != null || tdsMin != null) ...[
          const SizedBox(height: 6),
          Row(children: [
            Container(width: 16, height: 2,
              decoration: BoxDecoration(
                border: Border(bottom: BorderSide(color: Colors.green.withOpacity(0.7), width: 1.5,
                  style: BorderStyle.solid)),
              )),
            const SizedBox(width: 4),
            Text('Batas Normal', style: TextStyle(fontSize: 8, color: Colors.green.withOpacity(0.8))),
            if (phMin != null) ...[
              const SizedBox(width: 8),
              Text('(${phMin.toStringAsFixed(1)} – ${phMax!.toStringAsFixed(1)})',
                style: const TextStyle(fontSize: 8, color: AppColors.grey)),
            ] else if (tdsMin != null) ...[
              const SizedBox(width: 8),
              Text('(${tdsMin.toStringAsFixed(0)} – ${tdsMax!.toStringAsFixed(0)} ppm)',
                style: const TextStyle(fontSize: 8, color: AppColors.grey)),
            ],
          ]),
        ],
  ...
  // ── Ringkasan Statistik ──
  Widget _buildStatistikCard(List<dynamic> history, String kotak) {
    // Filter data kotor
    final phData = history
        .map((e) => (e['ph_avg'] as num?)?.toDouble())
        .where((v) => v != null && _isValidPh(v!))
        .cast<double>()
        .toList();
    final tdsData = history
        .map((e) => (e['tds_avg'] as num?)?.toDouble())
        .where((v) => v != null && _isValidTds(v!))
        .cast<double>()
        .toList();

    if (phData.isEmpty && tdsData.isEmpty) return const SizedBox.shrink();

    double phAvg  = phData.isEmpty  ? 0 : phData.reduce((a, b) => a + b)  / phData.length;
    double phMax  = phData.isEmpty  ? 0 : phData.reduce((a, b) => a > b ? a : b);
    double phMin  = phData.isEmpty  ? 0 : phData.reduce((a, b) => a < b ? a : b);
    double tdsAvg = tdsData.isEmpty ? 0 : tdsData.reduce((a, b) => a + b) / tdsData.length;
    double tdsMax = tdsData.isEmpty ? 0 : tdsData.reduce((a, b) => a > b ? a : b);
    double tdsMin = tdsData.isEmpty ? 0 : tdsData.reduce((a, b) => a < b ? a : b);

    // Indikator trend: bandingkan rata-rata 3 hari pertama vs 3 hari terakhir
    String phTrend = '→', tdsTrend = '→';
    Color phTrendColor = AppColors.grey, tdsTrendColor = AppColors.grey;
    if (phData.length >= 2) {
      final half   = (phData.length / 2).floor();
      final avgOld = phData.sublist(0, half).reduce((a, b) => a + b) / half;
      final avgNew = phData.sublist(half).reduce((a, b) => a + b) / (phData.length - half);
      if (avgNew > avgOld + 0.1)  { phTrend = '↑'; phTrendColor = Colors.green; }
      if (avgNew < avgOld - 0.1)  { phTrend = '↓'; phTrendColor = Colors.red; }
    }
    if (tdsData.length >= 2) {
      final half   = (tdsData.length / 2).floor();
      final avgOld = tdsData.sublist(0, half).reduce((a, b) => a + b) / half;
      final avgNew = tdsData.sublist(half).reduce((a, b) => a + b) / (tdsData.length - half);
      if (avgNew > avgOld + 10)  { tdsTrend = '↑'; tdsTrendColor = Colors.green; }
      if (avgNew < avgOld - 10)  { tdsTrend = '↓'; tdsTrendColor = Colors.red; }
    }

    final settPhMin  = (_setting['ph_min']  ?? 5.5).toDouble();
    final settPhMax  = (_setting['ph_max']  ?? 6.5).toDouble();
    final settTdsMin = (_setting['tds_min'] ?? 800).toDouble();
    final settTdsMax = (_setting['tds_max'] ?? 1200).toDouble();

    final phStatus  = (phAvg >= settPhMin && phAvg <= settPhMax);
    final tdsStatus = (tdsAvg >= settTdsMin && tdsAvg <= settTdsMax);

    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.border)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('📊 RINGKASAN PERIODE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.grey)),
        const SizedBox(height: 10),
        Row(children: [
          // pH column
          Expanded(child: Container(
            ...
            child: Column(children: [
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Text('pH', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AppColors.primary)),
                const SizedBox(width: 4),
                Text(phTrend, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: phTrendColor)),
              ]),
              const SizedBox(height: 6),
              Text(phAvg.toStringAsFixed(2),
                style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.text)),
              Text('rata-rata', style: const TextStyle(fontSize: 8, color: AppColors.grey)),
              const SizedBox(height: 6),
              _statRow('↑ Maks', phMax.toStringAsFixed(2), Colors.orange),
              _statRow('↓ Min',  phMin.toStringAsFixed(2), Colors.orange),
              const SizedBox(height: 4),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                decoration: BoxDecoration(
                  color: (phStatus ? AppColors.primary : Colors.orange).withOpacity(0.15),
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Text(phStatus ? '✓ Normal' : '⚠ Periksa',
                  style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700,
                    color: phStatus ? AppColors.primary : Colors.orange)),
              ),
            ]),
          )),
          const SizedBox(width: 10),
          ...
          Expanded(child: Container(
            ...
            child: Column(children: [
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Text('TDS', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AppColors.blue)),
                const SizedBox(width: 4),
                Text(tdsTrend, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: tdsTrendColor)),
              ]),
              const SizedBox(height: 6),
              Text('${tdsAvg.toStringAsFixed(0)} ppm',
                style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.text)),
              Text('rata-rata', style: const TextStyle(fontSize: 8, color: AppColors.grey)),
              const SizedBox(height: 6),
              _statRow('↑ Maks', '${tdsMax.toStringAsFixed(0)} ppm', Colors.orange),
              _statRow('↓ Min',  '${tdsMin.toStringAsFixed(0)} ppm', Colors.orange),
              const SizedBox(height: 4),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                decoration: BoxDecoration(
                  color: (tdsStatus ? AppColors.blue : Colors.orange).withOpacity(0.15),
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Text(tdsStatus ? '✓ Normal' : '⚠ Periksa',
                  style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700,
                    color: tdsStatus ? AppColors.blue : Colors.orange)),
              ),
            ]),
          )),
        ]),
  ...
  Widget _statRow(String label, String value, Color color) => Padding(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      Text(value, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: color)),
    ]),
  );
}
...
class _LineChartPainter extends CustomPainter {
  ...
  final Color color;
  final double minVal, maxVal, chartH;
  final double? phMin, phMax, tdsMin, tdsMax;
  ...
  _LineChartPainter({
    ...
    this.phMin, this.phMax, this.tdsMin, this.tdsMax,
  });

  double _yPos(double v, double w) {
    final range = maxVal - minVal == 0 ? 1.0 : maxVal - minVal;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawLimit(double val) {
      if (val < minVal || val > maxVal) return;
      final y = chartH - ((val - minVal) / range * chartH);
      const dashW = 6.0, gap = 4.0;
      double x = 0;
      while (x < w) {
        canvas.drawLine(Offset(x, y), Offset((x + dashW).clamp(0, w), y), limitPaint);
        x += dashW + gap;
      }
    }

    if (phMin != null) { drawLimit(phMin!); drawLimit(phMax!); }
    if (tdsMin != null) { drawLimit(tdsMin!); drawLimit(tdsMax!); }

    // ── Area fill ──
    final path = Path();
    for (int i = 0; i < n; i++) {
      final x = n > 1 ? i * step : w / 2;
      final y = _yPos(values[i], w);
      if (i == 0) path.moveTo(x, y); else path.lineTo(x, y);
    }
    final fillPath = Path.from(path)
      ..lineTo(n > 1 ? (n - 1) * step : w / 2, chartH)
      ..lineTo(0, chartH)
      ..close();
    ...
    final dotBgPaint   = Paint()..color = AppColors.card..style = PaintingStyle.fill;
    final outPaint     = Paint()..color = Colors.orange..style = PaintingStyle.fill;
    final outBgPaint   = Paint()..color = AppColors.card..style = PaintingStyle.fill;

    for (int i = 0; i < n; i++) {
      final x   = n > 1 ? i * step : w / 2;
      final y   = _yPos(values[i], w);
      final v   = values[i];
      final isOut = (phMin != null && (v < phMin! || v > phMax!)) ||
                    (tdsMin != null && (v < tdsMin! || v > tdsMax!));

      canvas.drawCircle(Offset(x, y), 5, isOut ? outBgPaint : dotBgPaint);
      canvas.drawCircle(Offset(x, y), 4, isOut ? outPaint : dotPaint);

      // Label di atas titik
      final tp = TextPainter(
        text: TextSpan(
          text: phMin != null ? v.toStringAsFixed(1) : v.toStringAsFixed(0),
          style: TextStyle(fontSize: 7, color: isOut ? Colors.orange : color, fontWeight: FontWeight.w700),
        ),
        textDirection: TextDirection.ltr,
      )..layout();
      tp.paint(canvas, Offset(x - tp.width / 2, (y - 14).clamp(0, chartH)));
    }

    // ── X-axis labels ──
    final labelStyle = const TextStyle(fontSize: 7, color: AppColors.grey);
    // Show every label if few, else every 2nd
    final step2 = n > 7 ? 2 : 1;
    for (int i = 0; i < n; i += step2) {
      ...
      tp.paint(canvas, Offset((x - tp.width / 2).clamp(0, w - tp.width), chartH + 4));
    }
  }
...
// Helper: buka URL di tab baru (Flutter Web)
void import_web_open(String url) {
  html.window.open(url, '_blank');
}

// ══════════════════════════════════════
// TAB 3 - POMPA STATUS (tidak berubah)
// ══════════════════════════════════════
  ...
  Widget _buildPompaCard(String key, Map<String, dynamic> info) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _buildInfoRow('Kotak', kotak.replaceAll('_', ' ').toUpperCase()),
        const SizedBox(height: 6),
      _buildInfoRow('Update terakhir', updAt.toString().replaceAll('T', ' ').split('.')[0]),
      ]),
    );
  }
...
// ══════════════════════════════════════
// TAB 4 - MANUAL KONTROL (tidak berubah)
// ══════════════════════════════════════
...
class _LogState extends State<LogPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetch() async {
    final l = await ApiService.getLog(
      limit: 50,
      trigger: _filterTrigger.isEmpty ? null : _filterTrigger,
      kotak: _filterKotak.isEmpty ? null : _filterKotak,
    );
  ...
  Widget _buildLogTab() => Column(children: [
    ...
    Container(
      ...
      child: Row(children: [
        const Text('Trigger: ', style: TextStyle(fontSize: 10, color: AppColors.grey)),
    ...
    Container(
      ...
      child: Row(children: [
        const Text('Kotak: ', style: TextStyle(fontSize: 10, color: AppColors.grey)),
        ...[('','Semua'), ('kotak_1','K1'), ('kotak_2','K2'), ('kotak_3','K3'), ('kotak_4','K4')].map((e) => Padding(
          padding: const EdgeInsets.only(right: 6),
          child: GestureDetector(
            onTap: () { setState(() => _filterKotak = e.$1); _fetch(); },
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
              decoration: BoxDecoration(
                color: _filterKotak == e.$1 ? AppColors.blue.withOpacity(0.2) : AppColors.bg,
                border: Border.all(color: _filterKotak == e.$1 ? AppColors.blue : AppColors.border),
                borderRadius: BorderRadius.circular(20),
              ),
              child: Text(e.$2, style: TextStyle(fontSize: 10,
                color: _filterKotak == e.$1 ? AppColors.blue : AppColors.grey)),
            ),
          ),
        )),
      ]),
              ...
              return Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(log['timestamp']?.toString().replaceAll('T', ' ').substring(0, 19) ?? '--',
  ...
  // ── Tab Rata-rata Mingguan (REVISED: filter data kotor) ──
        ...
        builder: (context, snap) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, i) {
              ...
              // Filter data kotor
              double? phAvg  = raw['ph_avg']  != null ? (raw['ph_avg']  as num).toDouble() : null;
              double? phMin  = raw['ph_min']  != null ? (raw['ph_min']  as num).toDouble() : null;
              double? phMax  = raw['ph_max']  != null ? (raw['ph_max']  as num).toDouble() : null;
              double? tdsAvg = raw['tds_avg'] != null ? (raw['tds_avg'] as num).toDouble() : null;
              double? tdsMin = raw['tds_min'] != null ? (raw['tds_min'] as num).toDouble() : null;
              double? tdsMax = raw['tds_max'] != null ? (raw['tds_max'] as num).toDouble() : null;

              // Tandai tidak wajar
              bool phOk  = phAvg  != null && _isValidPh(phAvg);
              bool tdsOk = tdsAvg != null && _isValidTds(tdsAvg);

              String phAvgStr  = phOk  ? phAvg!.toStringAsFixed(2)  : '—';
              String tdsAvgStr = tdsOk ? '${tdsAvg!.toStringAsFixed(0)} ppm' : '—';
              String phMinStr  = (phMin  != null && _isValidPh(phMin))  ? phMin!.toStringAsFixed(2)  : '—';
              String phMaxStr  = (phMax  != null && _isValidPh(phMax))  ? phMax!.toStringAsFixed(2)  : '—';
              String tdsMinStr = (tdsMin != null && _isValidTds(tdsMin)) ? '${tdsMin!.toStringAsFixed(0)} ppm' : '—';
              String tdsMaxStr = (tdsMax != null && _isValidTds(tdsMax)) ? '${tdsMax!.toStringAsFixed(0)} ppm' : '—';
              ...
              return Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    ...
                    if (raw['total_data'] != null)
                      Text('${raw['total_data']} data',
                        style: const TextStyle(fontSize: 10, color: AppColors.grey)),
                  ]),
                  const Divider(color: AppColors.border),
                  if (raw.isEmpty || raw['message'] != null)
                    ...
                    Column(children: [
                      ...
                      Container(
                        ...
                        child: Row(children: [
                          ...
                          Expanded(child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            _weeklyChip('Rata-rata', phAvgStr,  AppColors.primary),
                            _weeklyChip('Min',       phMinStr,  Colors.orange),
                            _weeklyChip('Maks',      phMaxStr,  Colors.orange),
                          ])),
                        ]),
                      ),
                      // TDS block
                      Container(
                        ...
                        child: Row(children: [
                          ...
                          const SizedBox(width: 8),
                          Expanded(child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            _weeklyChip('Rata-rata', tdsAvgStr, AppColors.blue),
                            _weeklyChip('Min',       tdsMinStr, Colors.orange),
                            _weeklyChip('Maks',      tdsMaxStr, Colors.orange),
                          ])),
                        ]),
                      ),
                    ]),
  ...
  Widget _weeklyChip(String label, String value, Color color) => Column(children: [
    Text(label, style: const TextStyle(fontSize: 9, color: AppColors.grey)),
    Text(value, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700,
      color: value == '—' ? AppColors.grey : color)),
  ]);
}

// ══════════════════════════════════════
// TAB 6 - PENGATURAN (tidak berubah)
// ══════════════════════════════════════
      ...
      Expanded(child: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          _buildSection('🔧 KALIBRASI POMPA', [
            _buildRow('Durasi Pompa pH (detik)', 'Lama pompa pH Up/Down nyala per pulse', _faktorPh),
            _buildRow('Jeda Mixing (detik)', 'Waktu tunggu setelah pulse sebelum cek ulang', _faktorTds),
          ]),
          const SizedBox(height: 12),
