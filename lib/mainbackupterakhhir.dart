...
// ══════════════════════════════════════
// MODEL — PERBAIKAN UTAMA DI SINI
// Sekarang phNormal & tdsNormal pakai nilai dari pengaturan,
// bukan hardcoded 50–300 ppm seperti versi lama.
// ══════════════════════════════════════
class SensorData {
  final String kotak;
  final double ph;
  ...
  // Parameter batas — default sesuai pengaturan umum hidroponik
  final double phMin;
  final double phMax;
  final double tdsMin;
  final double tdsMax;
...
this.tdsMax = 1000.0,
  ...
  // Factory lama — tetap ada supaya tidak ada yang patah,
  // tapi sekarang default tdsMin/tdsMax sudah benar
  ...
  // Factory baru — pakai nilai dari pengaturan server
      ...
      body: Center(
        child: SingleChildScrollView(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Text('Silakan login untuk melanjutkan', style: TextStyle(fontSize: 12, color: AppColors.grey)),
              ...
              child: Column(children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: IconButton(icon: Icon(_obscure ? Icons.visibility : Icons.visibility_off, color: AppColors.grey), onPressed: () => setState(() => _obscure = !_obscure)),
                    enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: AppColors.border), borderRadius: BorderRadius.circular(8)),
                    focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: AppColors.primary), borderRadius: BorderRadius.circular(8)),
                  ),
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                    child: _loading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text('LOGIN', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14)),
                  ),
                ),
              ]),
            ),
...
// ══════════════════════════════════════
// TAB 1 - DASHBOARD
// PERUBAHAN: _fetch() sekarang ambil pengaturan dulu,
// lalu pass ke SensorData.fromJsonWithBatas()
// ══════════════════════════════════════
...
class _DashboardState extends State<DashboardPage> {
  ...
  Future<void> _fetch() async {
    ...
    final setting = await ApiService.getPengaturan(); // ← BARU: ambil pengaturan

    if (mounted) setState(() {
      _loading = false;
      _setting = setting;
      // Gunakan fromJsonWithBatas supaya batas pH/TDS dari pengaturan dipakai
  ...
  Widget _buildKotak1() => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        const Text('🧪 KOTAK 1 — MIXING TANK', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800)),
        const Spacer(),
...
class _SensorState extends State<SensorPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetch() async {
    final s       = await ApiService.getSensorLatest();
    final setting = await ApiService.getPengaturan(); // ← BARU
  ...
  Widget _buildKotakDetail(String kotak, String label) {
    // PERUBAHAN: pakai fromJsonWithBatas, bukan fromJson
...
// ══════════════════════════════════════
// TAB 3 - POMPA STATUS (tidak ada perubahan)
// ══════════════════════════════════════
...
// ══════════════════════════════════════
// TAB 4 - MANUAL KONTROL (tidak ada perubahan)
// ══════════════════════════════════════
...
// ══════════════════════════════════════
// TAB 6 - PENGATURAN (tidak ada perubahan)
// ══════════════════════════════════════
